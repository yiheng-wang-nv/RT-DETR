...
from .layers.general_layers import SEModule
...
class TResNetBackbone(Module):
    def __init__(
        ...
        super(TResNetBackbone, self).__init__()
        ...
        if pretrained_path != "":
            ...
            weights = {k: v for k, v in weights.items() if k in self.state_dict().keys() and v.shape == self.state_dict()[k].shape}
            self.load_state_dict(weights, strict=False)
            print(f'Load {len(weights)} keys')
